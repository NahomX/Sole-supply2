 ...
 * old), and sends a Telegram message via the ops bot if any are found.
 *
 ...
 *   OPS_BOT_TOKEN        — ops bot token (replaces standalone TELEGRAM_BOT_TOKEN)
 *   TELEGRAM_CHAT_ID     — destination chat / channel ID
 *
 * Note: TELEGRAM_BOT_TOKEN is no longer used; OPS_BOT_TOKEN takes over.
 * Remove TELEGRAM_BOT_TOKEN from Vercel env vars once this PR merges and
 * PR #9 (feat/stale-checker) is retired.
 */
export async function GET(req: NextRequest) {
  ...
  // -----------------------------------------------------------------------
  // Build and send Telegram digest via the ops bot.
  // -----------------------------------------------------------------------
  // Use OPS_BOT_TOKEN (preferred) with TELEGRAM_BOT_TOKEN as fallback so
  // this works both before and after PR #9 (stale-checker) merges.
  const botToken = process.env.OPS_BOT_TOKEN ?? process.env.TELEGRAM_BOT_TOKEN;
  const chatId = process.env.TELEGRAM_CHAT_ID;

  if (!botToken || !chatId) {
    console.warn(
      `[stale-digest] ${stale.length} stale shoes found but bot token/chat ID are not set`
    );
